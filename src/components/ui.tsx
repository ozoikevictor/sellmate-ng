...
export function CheckoutHeader({
  ...
  const statusText = mode === "success" ? "Payment complete" : isCheckout ? "Delivery and payment" : "Shopping cart";
    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[#E5E7EB] bg-white shadow-sm">
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-3 sm:h-[68px] sm:px-5">
        <Link href={storeHref} className="flex min-w-0 items-center gap-2 text-base font-black leading-tight text-[#1F2937] sm:gap-3 sm:text-lg">
          <SellerLogo name={sellerName} logoUrl={sellerLogoUrl} size="sm" />
          <div className="min-w-0">
            <p className="truncate capitalize">{sellerName}</p>
            <p className="hidden text-xs font-bold text-slate-500 sm:block">{statusText}</p>
          </div>
        </Link>
        <div className="flex shrink-0 items-center gap-1 sm:gap-2">
          {isCheckout && cartHref ? <CartIconLink href={cartHref} count={cartCount ?? 0} label="Edit cart" /> : null}
          <Link href={storeHref} className="grid h-10 w-10 place-items-center rounded-full text-[#0F172A] transition hover:bg-[#F3F4F6] hover:text-[#16A34A]" aria-label={actionText} title={actionText}>
      ...
      <div className="border-t border-[#E5E7EB] bg-white px-3 py-2 sm:px-5">
        <div className="mx-auto max-w-7xl">
          <div className="flex h-11 items-center justify-between gap-3 rounded-full border border-[#E5E7EB] bg-[#F5F5F5] px-4 text-sm font-black text-[#1F2937]">
            <span className="flex min-w-0 items-center gap-2 truncate">
              <IconGlyph name="lock" className="h-4 w-4 shrink-0 text-[#16A34A]" />
              <span className="truncate">{statusText}</span>
            </span>
            <span className="shrink-0 rounded-full bg-white px-3 py-1 text-[11px] font-black text-[#16A34A] ring-1 ring-emerald-100">VENDORAQ</span>
          </div>
        </div>
      </div>
    </header>
