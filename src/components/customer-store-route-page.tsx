...
export function CustomerStoreRoutePage({ view }: { view: CustomerStoreView }) {
  ...
  async function cancelOrder(order: CustomerOrder) {
    ...
    setMessage("");
    const response = await fetch("/api/checkout/orders/cancel", {
      method: "POST",
    ...
    if (!response.ok) {
      setMessage(data.message ?? "Could not cancel this order. Please contact the seller.");
      setCancelingOrderId("");
      return;
    }

    updateCustomerOrder(order.orderId, { status: "Cancelled" });
    setCustomerOrders(readCustomerOrders(activeStoreSlug));
    setMessage("Order cancelled. The seller dashboard will show this order as cancelled.");
    setCancelingOrderId("");
  }
...
function OrdersView({ orders, cancelingOrderId, onCancelOrder }: { orders: CustomerOrder[]; cancelingOrderId: string; onCancelOrder: (order: CustomerOrder) => void }) {
          ...
          <article key={order.orderId} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                <span className={`rounded-full px-3 py-1 text-xs font-black ring-1 ${order.status === "Cancelled" ? "bg-rose-50 text-rose-800 ring-rose-100" : "bg-emerald-50 text-emerald-800 ring-emerald-100"}`}>{order.status}</span>
            ...
            <div className="mt-4 flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm font-semibold leading-6 text-slate-500">Delivery: {order.delivery_address}, {order.city}</p>
              {canCancel ? (
                <button
                  type="button"
                  onClick={() => onCancelOrder(order)}
                  disabled={isCanceling}
                  className="shrink-0 rounded-md border border-rose-200 bg-rose-50 px-4 py-2 text-xs font-black text-rose-700 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
                >
                  {isCanceling ? "Cancelling..." : "Cancel order"}
                </button>
              ) : (
                <span className="shrink-0 rounded-md bg-slate-100 px-4 py-2 text-xs font-black text-slate-500">
                  {order.status === "Cancelled" ? "Cancelled by customer" : "Delivery completed"}
                </span>
              )}
            </div>
          </article>
